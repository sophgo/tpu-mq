...
class SophgoTpuQuantizer(ModelQuantizer):
    ...
    def __init__(self, extra_quantizer_dict, extra_fuse_dict,quant_dict,chip):
        ...
        self.additional_output_node_name = []
    ...
    def _insert_fake_quantize_for_act_quant(
        ...
        for node in node_to_quantize_output:
            ...
            logger.info("Insert act 4 quant {}".format(quantizer_name2))
            with graph.inserting_after(node):
   ...
   def insert_node(self, model, node, new_node_name):
    ...
    def _find_act_quants(self, model: GraphModule) -> list:
        ...
        self.only_enable_ob = []
        logger.info("SophgoQ _find_act_quants end super")
        ...
        if self.strategy == 'CNN':
            for node in nodes:
                if node.name in ['qa_outputs','squeeze','squeeze_1']:
                    node_need_to_quantize_output.append(node)
                print(f'checking {node.op} {node.target} {node.name}')
                if (node.op == "call_module" and node.target in self.exclude_module_name) or \
                    ...
                    print(f'Skip exclude {node.name}')
                    continue
                if (node.op == "call_module" and isinstance(modules[node.target], module_type_to_quant_input)) or \
                   (node.op == "call_module" and isinstance(modules[node.target], self.module_type_to_quant_input_transformer)) or \
                    ((node.op == 'call_function' or node.op == 'call_method') and \
                        (node.target in function_type_to_quant_input or \
                        node.target in self.function_type_to_quant_input_transformer)):
                    for next_node in node.users:
                        if ((next_node.op == 'call_function' and next_node.target in self._passed_func_type) or
                            ...
                            if next_node not in node_need_to_quantize_output:
                                ...
                                print(">>>>> append user pass node ", next_node)
                                self.only_enable_ob.append(next_node.name)
                        else:
                            if node not in node_need_to_quantize_output:
                                node_need_to_quantize_output.append(node)
                                print(">>>>> append normal node ", node)
                                self.only_enable_ob.append(node.name)
                if (node.op == "call_module" and node.target in self.additional_output_node_name) or \
                    ((node.op == 'call_function' or node.op == 'call_method') and \
                        (node.target in self.additional_output_node_name)):
                        # to add selected quant output nodes
                    if node not in node_need_to_quantize_output:
                        node_need_to_quantize_output.append(node)
                        print(">>>>> append additional node ", node)
                        self.only_enable_ob.append(node.name)
                                      
        logger.info("SophgoQ _find_act_quants end strategy")
        print(node_need_to_quantize_output)
        ...
        logger.info("SophgoQ _find_act_quants after loop nodes")
        print(node_need_to_quantize_output)
