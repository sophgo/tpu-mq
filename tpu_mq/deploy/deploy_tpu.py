...
from tpu_mq.deploy.common import (
...
class LinearQuantizer_process(object):
    ...
    def get_correct_tpu_input_tensor_name(self, node, out2node, have_int4=False): #和tpu-mlir的命名风格一致
    ...
    def post_process_clip_ranges(self, clip_ranges, graph, inp2node, out2node):
        def find_the_closest_clip_range(node):
            tensor_name = self.get_correct_tpu_input_tensor_name(node, out2node)
        ...
        for node in graph.node:
            if node.op_type in ['Flatten', 'Resize']:
                ...
                if tensor_name:
                    new_name = self.get_correct_tpu_input_tensor_name(node, out2node)
        ...
        for node in graph.node:
            ...
            if tensor_name not in clip_ranges:
                ...
                while pre_op.op_type in op_type_inAndOutShouldSameClipRange:
                    tensor_name2 = self.get_correct_tpu_input_tensor_name(pre_op, out2node, have_int4)
        ...
        if first_call_in:
            for k in param_to_idx_dict:
                if int(data0) == param_to_idx_dict[k][0]:
                    ...
                    print(f'deploy_tpu, name: {k}, data0:{int(data0)}, unique_id: {unique_id}')
                    break
        return unique_id
...
remove_fakequantize_and_collect_params_tpu = LinearQuantizer_process().remove_fakequantize_and_collect_params
