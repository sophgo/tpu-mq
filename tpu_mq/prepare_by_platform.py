...
from tpu_mq.convert_deploy import convert_deploy, convert_merge_bn, deepcopy_graphmodule
from tpu_mq.utils.utils import generate_random_string
from tpu_mq.mlir.tpu_utils import param_to_idx_dict

from tpu_mq.fake_quantize import (
...
import tpu_mq
from tpu_mq.fuser_method_mappings import fuse_custom_config_dict
from tpu_mq.utils.logger import logger
from tpu_mq.utils.registry import DEFAULT_MODEL_QUANTIZER
from tpu_mq.scheme import QuantizeScheme

from tpu_mq.nn.intrinsic.qat.modules.conv_fused_tpu import ConvReLU2d_tpu, ConvBn2d_tpu, ConvBnReLU2d_tpu
...
def get_qconfig_by_platform(quant_dict:Dict,extra_qparams: Dict):
    ...
    # qconfig["object_type"] = {torch.nn.Linear:createQConfigForTpu_weight()} #int8 qat, TPU use sym per-layer
    ...
    if object_type:
        ...
        for type_name,type_data in object_type.items():
            ...
            if mode=="activation":
                ...
                weight = createQConfigForTpu_weight(chip, bit_num = bit).weight
                qconfig['object_type'][type_name]=createQConfigForTpu_activation(bit_num = bit, a_fakequantize = afq, a_observer = aob, weight=weight)
            elif mode=="weight":
                ...
                activation = createQConfigForTpu_activation(bit_num = 8).activation
                qconfig['object_type'][type_name]=createQConfigForTpu_weight(chip, bit_num = bit, w_fakequantize = wfq, w_observer = wob, wper_channel=wpc, activation=activation)
            else:
    ...
    if module_name:
        ...
        for type_name,type_data in module_name.items():
            ...
            if mode=="activation":
                ...
                qconfig["module_name"][type_name]=createQConfigForTpu_activation(bit_num = bit, a_fakequantize = afq, a_observer = aob)
            elif mode=="weight":
                ...
                qconfig["module_name"][type_name]=createQConfigForTpu_weight(chip, bit_num = bit, w_fakequantize = wfq, w_observer = wob, wper_channel=wpc)
            else:
                raise ValueError(f'无效的模式: {mode}。模式应该是 "activation" 或 "weight"。')
...
def createQConfigForTpu_activation(bit_num = 4, a_fakequantize = 'LearnableFakeQuantize', a_observer = 'MinMaxObserver', a_fakeq_params = {}, a_observer_extra_args = {}, weight=None):
    a_observer = ObserverDict[a_observer]
    a_fakequantize = FakeQuantizeDict[a_fakequantize]
    a_qscheme = QuantizeScheme(symmetry=True, per_channel=False, pot_scale=False, bit=bit_num) #TPU use sym per-layer
    a_qscheme.kwargs.update(a_observer_extra_args)
    a_qconfig = a_fakequantize.with_args(observer=a_observer, **a_fakeq_params, **a_qscheme.to_observer_params())
    return QConfig(activation=a_qconfig, weight=weight)

def createQConfigForTpu_weight(chip, bit_num = 4, w_fakequantize = 'FixedFakeQuantize', w_observer = 'MinMaxObserver', w_fakeq_params = {}, w_observer_extra_args = {}, wper_channel=False, activation=torch.nn.Identity):
    w_observer = ObserverDict[w_observer]
    w_fakequantize = FakeQuantizeDict[w_fakequantize]
    sym_range = False
    if chip in ['MARS3', 'CV183X', 'CV182X', 'CV181X', 'CV180X', 'CV186X']:
        sym_range = True
    w_qscheme = QuantizeScheme(symmetry=True, per_channel=wper_channel, pot_scale=False, bit=bit_num, symmetric_range = sym_range) #TPU use sym per-layer
    w_qscheme.kwargs.update(w_observer_extra_args)
    w_qconfig = w_fakequantize.with_args(observer=w_observer, **w_fakeq_params, **w_qscheme.to_observer_params())
...
def createQConfigForInt4TpuLiner(w_fakequantize = 'LearnableFakeQuantize', a_fakequantize = 'LearnableFakeQuantize', 
                w_observer = 'MinMaxObserver', a_observer = 'EMAMinMaxObserver', w_qscheme = {}, a_qscheme = {},
                w_fakeq_params = {}, a_fakeq_params = {}, w_observer_extra_args = {}, a_observer_extra_args = {}):
    w_observer = ObserverDict[w_observer]
    w_fakequantize = FakeQuantizeDict[w_fakequantize]
...
def prepare_dynamic_param_update(prepared: GraphModule, cuda: int = None, input_shape_dict: list = None, chip: str = "CV181X",  model_name: str = "tpu_mq_qmodel", net_type: str = "CNN"):
    print('>>>Gets the mapping between the pytorch param parameter and the parameter to be updated in the tpu-mlir...')
    module_tmp = deepcopy_graphmodule(prepared)
    module_tmp = module_tmp.cuda(cuda)
    module_tmp.eval()
    images = torch.zeros(*list(input_shape_dict.values())[0], device='cuda')
    module_tmp(images)
    ...
    for name, param in module_tmp.named_parameters():
        param = param.view(-1)
        old_data0 = copy.deepcopy(param.data[0])
        param.data[0] = idx
        unique_id = generate_random_string(30)
        print(f'prepare_by_platform, name: {name}, data0:{idx}, unique_id: {unique_id}, old_data0:{old_data0}')
        param_to_idx_dict[name] = [idx,unique_id]
        idx = idx + 1
    convert_deploy(module_tmp, net_type, input_shape_dict=input_shape_dict,
        model_name='{}'.format(model_name), not_gen_bmodel = True,
        output_path='/tmp/', bf16_mix_prec = False, mlir_deploy=False, chip=chip)
...
def prepare_by_platform(
    ...
    not_dynamic_update_param = False if 'not_dynamic_update_param' not in quant_dict else quant_dict['not_dynamic_update_param']
    use_tpu_mlir_fwd = False if 'use_tpu_mlir_fwd' not in quant_dict else quant_dict['use_tpu_mlir_fwd']
    net_type = 'CNN' if 'strategy' not in quant_dict else quant_dict['strategy']
    model_name = "tpu_mq_qmodel" if 'model_name' not in quant_dict else quant_dict['model_name']
    ...
    # Prepare
    import tpu_mq.custom_quantizer  # noqa: F401
    ...
    if (not not_dynamic_update_param) and use_tpu_mlir_fwd:
        prepare_dynamic_param_update(prepared, cuda, input_shape_dict, chip, model_name, net_type)
    return prepared
