...
from tpu_mq.convert_deploy import convert_deploy, convert_onnx, export_onnx_with_fakequant_node
...
import hashlib
from tpu_mq.mlir.mlir_utils import hash_mlir
from tpu_mq.mlir.mlir_utils import tpu_train_prepare
from tpu_mq.mlir.mlir_utils import update_mlir_weight
from tpu_mq.mlir.mlir_utils import lower_net
...
parser.add_argument('--test_mlir', dest='test_mlir', action='store_true',
                    help='')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.test_mlir:
        ...
        global name_dict
        global need_transpose
        global mlir_weight_file
        succ, name_dict, need_transpose, mlir_weight_file = tpu_train_prepare(model, args, val_loader)
        t2=datetime.datetime.now()
        print(f'time to prepare {t2-t1} {mlir_weight_file}')
        if succ:
            print('TRAIN PREPARE SUCCESS')
        else:
            print('TRAIN PREPARE FAIL')
            sys.exit(1)
        t1=datetime.datetime.now()            
        update_mlir_weight(model, need_transpose, mlir_weight_file, name_dict)
        t2=datetime.datetime.now()
        print(f'update weight {t2-t1}')
        int8_module = lower_net(args.arch, args.chip)
        t1=datetime.datetime.now()
        print(f'update weight {t1-t2}')
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    print(f'weight file in train {mlir_weight_file}')
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        update_mlir_weight(model, need_transpose, mlir_weight_file, name_dict)
        int8_module = lower_net(args.arch, args.chip)
        int8_module.set_tensor(int8_module.inputs[0],images)
        int8_module.invoke()
        tensors = int8_module.get_tensor(output).copy()
        # compute output
        '''
        output = model(images)
        '''
        loss = criterion(torch.tensor(tensors), target)

        # measure accuracy and record loss
        acc1, acc5 = accuracy(tensors, target, topk=(1, 5))
