...
from tpu_mq.nn.intrinsic import LinearBn1d
import torch.nn.intrinsic as nni


class LinearBn1d_tpu(Linear, _FusedModule):
    _version = 2
...
class LinearReLU_tpu(Linear_tpu):
    r"""
    ...
    def __init__(self, in_features, out_features, bias=True,
                 qconfig=None):
        super(LinearReLU_tpu, self).__init__(in_features, out_features, bias, qconfig)

    def forward(self, input):
        return F.relu(Linear_tpu._forward(self, input))

    @classmethod
    def from_float(cls, mod):
        return super(LinearReLU_tpu, cls).from_float(mod)
