...
from tpu_mq.deploy import (
    ...
    remove_fakequantize_and_collect_params_tpu,
)
...
try:
    ...
    from tpu_mq.mlir.tpu_utils import deploy_to_mlir
        ...
        try:
            ...
            try:
                torch.onnx.export(model, dummy_input, onnx_model_path,
                                ...
                                custom_opsets={'tpu_custom' : opset_version})
...
def export_qtable_tf(context_filename, model_name, output_path, quant_mode):
        ...
        file_h.close()
        q_table = osp.join(output_path, '{}_q_table_from_tpu_mq_{}'.format(model_name, quant_mode))
        with open(q_table, 'w') as f:
            f.write("# qtable from tpu_mq\n")
...
def deploy_qparams_Academic_NLP(model: GraphModule, onnx_model_path, model_name, **kwargs):
    ...
    if quant_mode == "FP8":
        ...
        file_h.close()
        cali_table = osp.join(output_path, '{}_float_cali_table_from_tpu_mq_Academic_NLP'.format(model_name))
        fp8_header = "tpu_mq-fp8"
        ...
        file_h.close()
        cali_table = osp.join(output_path, '{}_cali_table_from_tpu_mq_Academic_NLP'.format(model_name))
...
@register_deploy_function("CNN")
def deploy_qparams_tpu(model: GraphModule, onnx_model_path, model_name, quant_type_dict, **kwargs):
    logger.info("Extract qparams for tpu.")
    cali_mode = "tpu"
    global model_onnx_mem
    export_to_mem = 'not_gen_bmodel' in kwargs and kwargs['not_gen_bmodel']
    mlir_deploy = 'mlir_deploy' in kwargs and kwargs['mlir_deploy']
    if not export_to_mem:
        model_onnx_mem = None
    model_onnx_mem = remove_fakequantize_and_collect_params_tpu(onnx_model_path, model_name, quant_type_dict, model_onnx_mem, mlir_deploy)
    # print("导出calitable")
    output_path = osp.dirname(onnx_model_path)
    context_filename = osp.join(output_path, '{}_clip_ranges.json'.format(model_name))
    file_h = open(context_filename, "r")
    ...
    file_h.close()
    cali_table = osp.join(output_path, '{}_cali_table_from_tpu_mq'.format(model_name))
    work_mode = kwargs.get('work_mode', 'QAT_all_int8')
    if work_mode not in  ['QAT_all_int8', 'int4_and_int8_mix', 'int4_and_int8_mix_no_fc']:
        print('QAT_all_int8 not in [QAT_all_int8, int4_and_int8_mix, int4_and_int8_mix_no_fc],set to QAT_all_int8')
        work_mode = 'QAT_all_int8'
...
def convert_deploy(model: GraphModule, net_type='CNN',
                   ...
                   model_name='tpu_mq_qmodel', deploy_to_qlinear=False, mlir_deploy=False, chip="BM1690", val_loader=None, **extra_kwargs):
    ...
    Args:
        ...
        model_name (str, optional): name of converted onnx model. Defaults to 'tpu_mq_qmodel'.
    ...
    >>> note on input_shape_dict:
        ...
        while forward function signature is like:
                def forward(self, input_0, input_1):
                    pass
    """
    if 'param_check' in extra_kwargs and extra_kwargs['param_check']:
    ...
    if mlir_deploy and chip != 'academic':
        return deploy_to_mlir(chip, val_loader, **kwargs)

def export_onnx_with_fakequant_node(model: GraphModule, net_type='CNN',
                   input_shape_dict=None, dummy_input=None, output_path='./',
                   model_name='tpu_mq_qmodel', deploy_to_qlinear=False, **extra_kwargs):
